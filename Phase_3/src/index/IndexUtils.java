...
public class IndexUtils {
	 ...
	 */
	public static IndexFileScan ClBTree_scan(CondExpr[] selects, IndexFile indFile) 
			throws IOException, 
			...
			ConstructPageException
	{
		...
		if (selects == null || selects[0] == null) {
			indScan = ((ClusteredBTreeFile)indFile).new_scan_cl(null, null);
			return indScan;
		}
		...
		if (selects[1] == null) {
			...
			if (selects[0].op.attrOperator == AttrOperator.aopEQ) {
				if (selects[0].type1.attrType != AttrType.attrSymbol) {
					...
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl(key, key);
				}
				else {
			...
			if (selects[0].op.attrOperator == AttrOperator.aopLT || selects[0].op.attrOperator == AttrOperator.aopLE) {
				if (selects[0].type1.attrType != AttrType.attrSymbol) {
					key = getValue(selects[0], selects[0].type1, 1);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl(null, key);
				}
				else {
					key = getValue(selects[0], selects[0].type2, 2);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl(null, key);
			...
			if (selects[0].op.attrOperator == AttrOperator.aopGT || selects[0].op.attrOperator == AttrOperator.aopGE) {
				if (selects[0].type1.attrType != AttrType.attrSymbol) {
					key = getValue(selects[0], selects[0].type1, 1);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl(key, null);
				}
				else {
					key = getValue(selects[0], selects[0].type2, 2);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl(key, null);
			...
			case AttrType.attrString:
				if (((StringKey)key1).getKey().compareTo(((StringKey)key2).getKey()) < 0) {
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl(key1, key2);
				}
				else {
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl(key2, key1);
				}
				return indScan;
	 ...
	 * @exception ConstructPageException failed to construct a header page
	 */
	public static IndexFileScan ClBTree_scan_ASC(CondExpr[] selects, IndexFile indFile) 
			throws IOException, 
			UnknownKeyTypeException, 
			InvalidSelectionException,
			KeyNotMatchException,
			...
			ConstructPageException
	{
		...
		if (selects == null || selects[0] == null) {
			indScan = ((ClusteredBTreeFile)indFile).new_scan_cl_ASC(null, null);
			return indScan;
		}
		...
		if (selects[1] == null) {
			...
			if (selects[0].op.attrOperator == AttrOperator.aopEQ) {
				if (selects[0].type1.attrType != AttrType.attrSymbol) {
					key = getValue(selects[0], selects[0].type1, 1);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl_ASC(key, key);
				}
				else {
					key = getValue(selects[0], selects[0].type2, 2);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl_ASC(key, key);
			...
			if (selects[0].op.attrOperator == AttrOperator.aopLT || selects[0].op.attrOperator == AttrOperator.aopLE) {
				if (selects[0].type1.attrType != AttrType.attrSymbol) {
					key = getValue(selects[0], selects[0].type1, 1);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl_ASC(null, key);
				}
				else {
					key = getValue(selects[0], selects[0].type2, 2);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl_ASC(null, key);
			...
			if (selects[0].op.attrOperator == AttrOperator.aopGT || selects[0].op.attrOperator == AttrOperator.aopGE) {
				if (selects[0].type1.attrType != AttrType.attrSymbol) {
					key = getValue(selects[0], selects[0].type1, 1);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl_ASC(key, null);
				}
				else {
					key = getValue(selects[0], selects[0].type2, 2);
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl_ASC(key, null);
			...
			case AttrType.attrString:
				if (((StringKey)key1).getKey().compareTo(((StringKey)key2).getKey()) < 0) {
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl_ASC(key1, key2);
				}
				else {
					indScan = ((ClusteredBTreeFile)indFile).new_scan_cl_ASC(key2, key1);
				}
				return indScan;
