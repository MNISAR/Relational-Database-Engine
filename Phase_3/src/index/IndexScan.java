...
public class IndexScan extends Iterator {
	...
	public IndexScan(
		...
		// For hash index, first element of selects should provide the condition.
		// and type2 must cannot be symbol.
		try {
		...
		case IndexType.B_Index:
			...
			break;
		case IndexType.CL_B_Index_nodes:
			// error check the select condition
			// must be of the type: value op symbol || symbol op value
			// but not symbol op symbol || value op value
			try {
		...
		case IndexType.Cl_B_Index_DESC:
			// error check the select condition
			// must be of the type: value op symbol || symbol op value
			// but not symbol op symbol || value op value
			try {
			...
			try {
				indScan = (ClBTFileScan) IndexUtils.ClBTree_scan(selects, indFile);
			} catch (Exception e) {
		...
		case IndexType.Hash:
			try {
				switch (_selects[0].type2.attrType) {
				case AttrType.attrInteger:
					search_key = new HashKey(_selects[0].operand2.integer);
					break;
				case AttrType.attrReal:
					search_key = new HashKey(_selects[0].operand2.real);
					break;
				case AttrType.attrString:
					search_key = new HashKey(_selects[0].operand2.string);
					break;
				default:
					System.out.println("Cannot use type " + _selects[0].type2.toString());
				}
				HindFile = new HIndex(indName);
				Hscan = new HIndexScan(HindFile, search_key);

			} catch (Exception e) {
				System.out.println("Cannot open H-index file!");
			}
			break;
		default:
	...
	public Tuple get_next()
			...
			IOException 
	{
		if (_index_type.indexType == IndexType.Hash) {
			try{
				HashEntry ent;
				while((ent = Hscan.get_next())!=null){
					tuple1 = f.getRecord(ent.rid);
					try {
			...
			while (nextentry != null) {
				if (index_only) {
					...
					if (_types[_fldNum - 1].attrType == AttrType.attrInteger) {
						...
						// attrReal not supported for now, its supported now !
	...
	public void close() throws IOException, IndexException {
		if(_index_type.indexType==IndexType.Hash){
			Hscan.closeHIndexScan();
			try{
				HindFile.close();
			}catch (Exception e){
				System.out.println("Cannot close HindFile!");
			}
	...
	public KeyDataEntry get_next_key_data() throws ScanIteratorException {
		if ( _index_type.indexType == IndexType.CL_B_Index_nodes ) {
			return indScan.get_next();
		}
		// TODO Auto-generated method stub
		return null;
	}
