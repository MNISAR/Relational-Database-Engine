...
public class SortedHeapfile implements Filetype, GlobalConst {

  private PageId      _firstDirPageId;
  private HFPage      _firstDirPage;
  private int         _ftype;
  ...
  */
  ...
  public  SortedHeapfile(String name) 
      ...
      if(_firstDirPageId==null)
	{
	  ...
	  _firstDirPage = new HFPage();
	  _firstDirPage.init(_firstDirPageId, apage);
	  PageId pageId = new PageId(INVALID_PAGE);
	  
	  _firstDirPage.setNextPage(pageId);
	  _firstDirPage.setPrevPage(pageId);
  ...
  public RID insertRecord(byte[] recPtr) 
	   ...
	   IOException
    {
	    ...
	    PageId pageId = new PageId(INVALID_PAGE);
	    // we have just created the heapfile
	    if ( _firstDirPage.getNextPage() != pageId ) {
	    	System.out.println("file created and empty");
